Filter = do ->
  ...
  Filter::_buildUI = ->
    ...
    @_clearSelectionButton = $('<button class=clear></button>').text('Clear').attr('type', 'button').insertAfter(@_summaryElement)
  ...
  Filter::_updateSummary = ->
    ...
    if checked.length > 0 and checked.length < @_checkboxes().length
      summary = @_labelsFor(checked).join(', ')
